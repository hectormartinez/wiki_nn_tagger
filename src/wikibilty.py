...
def main():
    ...
    parser.add_argument("--dict", help="word,pos tab-separated dictionary", required=True)
    ...
    if args.train and len( args.train ) != 0:
        tagger.fit(args.train, args.iters, args.trainer,args.dict, dev=args.dev)
        if args.save:
            save(tagger, args)
...
class NNTagger(object):
    ...
    def fit(self, list_folders_name, num_iterations, train_algo, dictionary,dev=None):
        """
        train the tagger
        """
        ...
        print("task2t2i",task2t2i)
        label_lexicon = self.get_label_lexicon(dictionary,w2i,task2t2i["task0"])
        if dev:
        ...
        for iter in range(num_iterations):
            ...
            for ((word_indices,char_indices),y, task_of_instance) in train_data:
                ...
                loss1 = dynet.esum([self.other_loss_function(pred,gold,word_id,label_lexicon) for pred, gold,word_id in zip(output, y,word_indices)])
                #print("loss array",[self.pick_neg_log(pred,gold) for pred, gold in zip(output, y)])
                lv = loss1.value()
                total_loss += lv
